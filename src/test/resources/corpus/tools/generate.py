...
def positive_vectors(keys) -> list[dict]:
    ...
    out.append(vec(
        ...
        kind="manifest",
        description="Migration adopted successfully under the AMB-32 isolated-state rule. The announcing manifest at the original origin carries a migration_pointer to a successor origin and is the global current publication before successor verification. The successor manifest (in extra_files) is signed by the same K_publisher, binds to the successor address by the Tor v3 derivation, and is itself valid at clock_now (canary fresh, origin not expired). Its history-dependent Stage 8 checks use provisional state isolated from the announcing manifest and global publisher history; only confirmed adoption commits that state. The migration adoption outcome is accept even though announcing and successor manifests have the same canary issued_at and runtime_pubkey but distinct signed payloads. Accept counterpart to the migration rejects (200-204).",
        spec_refs=["§06", "§10"],
        verdict="accept",
...
def negative_vectors(keys) -> list[dict]:
    ...
    # ---- 159-stage5-budget-origin-precedence (AMB-28) ----
    # A manifest with TWO co-occurring Stage 5 cross-field violations: the same
    # over-budget state_policy as 143 (E_SUBMIT_BUDGET) AND origin.not_after
    # equal to canary.issued_at (E_ORIGIN_INVALID, as in 176). AMB-28 pins the
    # intra-Stage-5 order: per-field checks (incl. updated future-skew), then the
    # state_policy submit-budget aggregate (E_SUBMIT_BUDGET), then the
    # origin.not_after constraint (E_ORIGIN_INVALID), then migration_pointer. So
    # the budget violation is reported first. This vector deliberately carries
    # two live Stage 5 violations to pin the precedence cross-implementation (a
    # reading that checked origin before budget would report E_ORIGIN_INVALID).
    m_159 = make_manifest(
        publisher_priv=pp, publisher_pub=pp_pub,
        origin_pub=op_pub, runtime_pub=rp_pub,
        state_policy=state_policy_overflow,
        not_after="2026-05-07T00:00:00Z",  # == canary.issued_at -> E_ORIGIN_INVALID
    )
    out.append(vec(
        "159-stage5-budget-origin-precedence",
        kind="manifest",
        description=(
            "Manifest carrying two co-occurring Stage 5 cross-field violations: "
            "the over-budget state_policy of vector 143 (aggregate request_state "
            "66879 > 53248 state_budget, E_SUBMIT_BUDGET) AND origin.not_after "
            "equal to canary.issued_at (E_ORIGIN_INVALID, as in 176). AMB-28 "
            "pins the intra-Stage-5 order so the submit-budget aggregate is "
            "evaluated before the origin.not_after constraint; the reported "
            "diagnostic is therefore E_SUBMIT_BUDGET, not E_ORIGIN_INVALID. The "
            "manifest is signed correctly; both violations are live, which is "
            "the point of this precedence vector."
        ),
        spec_refs=["§06", "§07", "§09", "§10", "§11"],
        verdict="reject",
        diagnostic="E_SUBMIT_BUDGET",
        diagnostic_details={
            "component": "state",
        ...
        body_obj=m_159,
        context={"fetched_origin_address": m_159["origin"]["address"]},
    ))
    ...
    # ---- 193-string-unassigned-codepoint (Stage 5, E_SCHEMA_FIELD_SYNTAX; AMB-24)
    # A user-visible string containing U+0378, a code point reserved (unassigned)
    # in Unicode 15.0. Per §04 the assigned-only gate rejects any code point
    # unassigned in the pinned Unicode 15.0 baseline, before the NFC check. U+0378
    # has canonical combining class 0 and no decomposition, so it passes NFC
    # itself; only the assigned-only gate rejects it, which makes this vector
    # exercise the gate rather than NFC (distinct from 190-192, which exercise NFC
    # on assigned characters). Re-signed under K_publisher.
    m_unassigned = make_manifest(
        publisher_priv=pp, publisher_pub=pp_pub,
        origin_pub=op_pub, runtime_pub=rp_pub,
    )
    del m_unassigned["sig"]
    m_unassigned["canary"]["statement"] = "No warrants received.͸"
    m_unassigned["sig"] = sign(pp, CTX_MANIFEST, m_unassigned)
    out.append(vec(
        "193-string-unassigned-codepoint",
        kind="manifest",
        description=(
            "Manifest whose canary.statement contains U+0378, a code point "
            "reserved (unassigned) in Unicode 15.0. Per §04 (AMB-24) a "
            "user-visible string MUST contain only code points assigned in the "
            "pinned Unicode 15.0 baseline; an unassigned code point is rejected "
            "at Stage 5 as E_SCHEMA_FIELD_SYNTAX, before the NFC check. U+0378 "
            "has combining class 0 and no decomposition, so it passes NFC "
            "itself; only the assigned-only gate rejects it (distinct from "
            "190-192, which exercise NFC on assigned characters). Re-signed "
            "under K_publisher so the signature verifies and the gate is the "
            "only live violation."
        ),
        spec_refs=["§04", "§08", "§11"],
        verdict="reject",
        diagnostic="E_SCHEMA_FIELD_SYNTAX",
        body_obj=m_unassigned,
        context={"fetched_origin_address": m_unassigned["origin"]["address"]},
    ))

    # ---- 194-manifest-updated-impossible-date (Stage 5, E_SCHEMA_FIELD_SYNTAX; §04) ----
    # `updated` matches the YYYY-MM-DDTHH:MM:SSZ shape but denotes February 30,
    # which is not a real instant. Per §04 RFC 3339 timestamp validity, such a
    # value is rejected at Stage 5 schema validation as E_SCHEMA_FIELD_SYNTAX.
    m_194 = make_manifest(
        publisher_priv=pp, publisher_pub=pp_pub,
        origin_pub=op_pub, runtime_pub=rp_pub,
        updated="2026-02-30T00:00:00Z",
    )
    out.append(vec(
        "194-manifest-updated-impossible-date",
        kind="manifest",
        description=(
            "Manifest whose `updated` matches the YYYY-MM-DDTHH:MM:SSZ shape but "
            "denotes an impossible calendar date (2026-02-30, February 30). Per "
            "§04 RFC 3339 timestamp validity, a value that matches the lexical "
            "shape but does not denote a real instant is rejected at Stage 5 "
            "schema validation as E_SCHEMA_FIELD_SYNTAX. The manifest is signed "
            "correctly and otherwise well-formed; the calendrical violation is "
    ...
    m_195 = make_manifest(
        ...
        next_expected="2026-03-15T00:00:00Z",
    )
    out.append(vec(
        "195-canary-issued-at-impossible-date",
        kind="manifest",
        description=(
            ...
            "and otherwise well-formed; the calendrical violation is the only "
            "live violation."
        ),
        spec_refs=["§04", "§08", "§11"],
        verdict="reject",
        ...
        body_obj=m_195,
        context={"fetched_origin_address": m_195["origin"]["address"]},
    ))
    ...
    # ---- 123/124-link-*-url-userinfo (Stage 5, E_SCHEMA_FIELD_SYNTAX) ----
    # Both external URL kinds prohibit an RFC 3986 userinfo component. The
    # citation form is the familiar trusted.example@evil.example phishing
    # construction. The carrier form places an apparent clearnet identity
    # before @ and a valid onion v3 host after it; validators that strip
    # userinfo before onion-host validation used to accept it. Both documents
    # are otherwise valid and are signed correctly under K_runtime.
    c_citation_userinfo = make_content(
        runtime_priv=rp,
        path="/links/citation-userinfo",
        title="Citation userinfo",
        blocks=[{
            "kind": "link",
    ...
    out.append(vec(
        "123-link-citation-url-userinfo",
        kind="content",
        description=(
            "Content document with a citation target URL using the RFC 3986 "
            "userinfo phishing form https://trusted.example@evil.example/reference. "
            "The scheme, host, length, and character syntax are otherwise valid, "
            "but §03 prohibits every userinfo component in citation and carrier "
            "targets. Rejected at Stage 5 as E_SCHEMA_FIELD_SYNTAX. Signed "
            "correctly under K_runtime so userinfo is the only live violation."
        ),
        spec_refs=["§03", "§11"],
        verdict="reject",
        diagnostic="E_SCHEMA_FIELD_SYNTAX",
        body_obj=c_citation_userinfo,
        context={
            "fetched_path": c_citation_userinfo["path"],
            "expected_runtime_pubkey": b64u(rp_pub),
        },
    ))

    c_carrier_userinfo = make_content(
        runtime_priv=rp,
        path="/links/carrier-userinfo",
        title="Carrier userinfo",
        blocks=[{
            "kind": "link",
            "label": [{"kind": "text", "value": "Carrier", "marks": []}],
            "target": {
                "kind": "carrier",
                "carrier": "tor-v3",
                "url": f"http://trusted.example@{onion_address(op_pub)}/reference",
            },
        }],
    )
    out.append(vec(
        "124-link-carrier-url-userinfo",
        kind="content",
        description=(
            "Content document with a carrier target URL whose authority is "
            "trusted.example@<valid-onion-v3-host>. The post-@ host is a valid "
            "carrier address and every other URL constraint is satisfied, but "
            "§03 prohibits userinfo and requires rejection rather than stripping "
            "it before onion-host validation. Rejected at Stage 5 as "
            "E_SCHEMA_FIELD_SYNTAX. Signed correctly under K_runtime so userinfo "
            "is the only live violation."
        ),
        spec_refs=["§03", "§11"],
        verdict="reject",
        diagnostic="E_SCHEMA_FIELD_SYNTAX",
        body_obj=c_carrier_userinfo,
        context={
            "fetched_path": c_carrier_userinfo["path"],
            "expected_runtime_pubkey": b64u(rp_pub),
        },
    ))

    # ---- 125-state-updates-duplicate (Stage 5, E_STATE_DUPLICATE) ----
    # Both operation forms are individually valid, but they target the same
    # composite state key. The transaction is signed correctly so the duplicate
    # pair is the only live violation and must be detected before Stage 6.
    t_state_duplicate, _ = make_transaction(
        runtime_priv=rp,
        state_updates=[
            {
                "op": "set",
    ...
    out.append(vec(
        "125-state-updates-duplicate",
        kind="transaction",
        description=(
            "Transaction whose state_updates contains a valid set followed by "
            "a valid delete for the same (session, data) pair. §07 requires each "
            "(namespace, key) pair to occur at most once regardless of operation "
            "form, so Stage 5 rejects the whole transaction as E_STATE_DUPLICATE. "
            "The document is signed correctly under K_runtime; the duplicate pair "
            "is the only live violation."
        ),
        spec_refs=["§07", "§10", "§11"],
        verdict="reject",
        diagnostic="E_STATE_DUPLICATE",
        diagnostic_details={
            "duplicate_namespace": "session",
            "duplicate_key": "data",
        },
        body_obj=t_state_duplicate,
        context={"expected_runtime_pubkey": b64u(rp_pub)},
    ))
    ...
    # ---- 129-schema-block-kind-integer (Stage 5, E_SCHEMA_FIELD_TYPE) ----
    # Correctly-signed content document with one block whose `kind`
    # discriminator is the integer 0 instead of a string. The block, inline,
    # link, form-field `kind` and the state-operation `op` are all closed
    # internally-tagged enum discriminators, and the discriminator MUST be a
    # string. An integer discriminator is a Stage 5 type error
    # (E_SCHEMA_FIELD_TYPE), reported before Stage 6 signature verification.
    # The document is otherwise valid and validly signed, so the integer
    # discriminator is the only live violation: an implementation that admits a
    # non-string discriminator (for example by mapping the integer to an enum
    # variant by index) would accept this document, where it MUST be rejected.
    c_int_kind = make_content(
        runtime_priv=rp,
        path="/articles/int-kind",
        title="Integer block kind",
        blocks=[{
            "kind": 0,
            "content": [
    ...
    out.append(vec(
        "129-schema-block-kind-integer",
        kind="content",
        description=(
            "Content document with one block whose `kind` discriminator is the "
            "integer 0 instead of a string. The block/inline/link/form `kind` "
            "and the state-operation `op` are closed internally-tagged enum "
            "discriminators that MUST be strings; an integer discriminator is a "
            "Stage 5 type error reported before Stage 6 signature verification "
            "(§10 first-failing-stage). The document is otherwise valid and "
            "validly signed, so an implementation that maps the integer to an "
            "enum variant by index would wrongly accept it. E_SCHEMA_FIELD_TYPE."
        ),
        spec_refs=["§02", "§03", "§11"],
        verdict="reject",
        diagnostic="E_SCHEMA_FIELD_TYPE",
        body_obj=c_int_kind,
        context={
            "fetched_path": "/articles/int-kind",
            "expected_runtime_pubkey": b64u(rp_pub),
        },
    ))
    ...
    # ---- 187-canary-runtime-pubkey-small-order (Stage 8, E_CANARY_INVALID; AMB-23)
    # A manifest whose canary.runtime_pubkey is the encoded identity point
    # (SMALL_ORDER_A, the same small-order Ed25519 point used by 153/179), the
    # rest valid and re-signed under the real K_publisher so Stage 6 passes. The
    # manifest is not verified under runtime_pubkey, so the small-order key would
    # otherwise surface only on the first content fetch; per AMB-23 (§05 strict
    # profile, §08) it is rejected structurally at Stage 8 as E_CANARY_INVALID
    # with details.reason="public_key_rejected". Canary timing is valid and no
    # publisher history is seeded, so the strict-profile check is the only live
    # Stage 8 violation.
    m_rtkey = make_manifest(
        publisher_priv=pp, publisher_pub=pp_pub,
        origin_pub=op_pub, runtime_pub=rp_pub,
    )
    del m_rtkey["sig"]
    m_rtkey["canary"]["runtime_pubkey"] = b64u(SMALL_ORDER_A)
    m_rtkey["sig"] = sign(pp, CTX_MANIFEST, m_rtkey)
    out.append(vec(
        "187-canary-runtime-pubkey-small-order",
        kind="manifest",
        description=(
            "Manifest whose canary.runtime_pubkey is the encoded Ed25519 "
            "identity point (a small-order point, the same encoding family as "
            "vectors 153/179), otherwise valid and re-signed under the real "
            "K_publisher so Stage 6 verifies. The manifest is signed under "
            "K_publisher, not runtime_pubkey, so the bad runtime key does not "
            "affect manifest signature verification. Per AMB-23 the §05 strict "
            "profile is enforced structurally on the declaring manifest at "
            "Stage 8: rejected as E_CANARY_INVALID with "
            "details.reason='public_key_rejected', rather than surfacing only as "
            "E_SIG_VERIFICATION on the first content document verified under "
            "K_runtime. Canary timing is valid and no publisher history is "
            "seeded, so the strict-profile check is the only live violation."
        ),
        spec_refs=["§05", "§08", "§11"],
        verdict="reject",
        diagnostic="E_CANARY_INVALID",
        ...
        body_obj=m_rtkey,
        context={"fetched_origin_address": m_rtkey["origin"]["address"]},
    ))
    ...
    # Observed-only mismatch: §10:298 requires the client to create an
    # automatic observation record at first contact, once the pipeline through
    # Stage 9 succeeds and with NO user decision; the record "is a retained
    # observation used to detect later changes". The pin prompt remains
    # unanswered, so the trust state never left First contact (§10:308 forbids
    # a passive pin) - yet a later manifest for the same site presenting a
    # different K_publisher.pub MUST resolve as Changed/mismatch, exactly as
    # against a pinned record. context.retained_provenance = "observed"
    # distinguishes this retention flavor from the pinned records of 210/211
    # (absent retained_provenance means pinned). The manifest is the same
    # correctly-signed second-publisher manifest as 210: the only live failure
    # is the identity mismatch against the observed first publisher.
    out.append(vec(
        "215-trust-observed-mismatch",
        kind="manifest",
        description="Manifest presenting a different K_publisher.pub than the identity the client automatically observed (but never pinned) for this site. Per \u00a710:298 the first-contact observation record is created with no user decision and is retained to detect later changes, so the mismatch fires exactly as against a pinned record even though the user never answered the pin prompt. The manifest is signed correctly under the second publisher key; the identity mismatch is the Stage 6 pre-check failure, taking precedence over signature verification. Rejected with E_TRUST_MISMATCH.",
        spec_refs=["\u00a710", "\u00a711"],
        verdict="reject",
        diagnostic="E_TRUST_MISMATCH",
        body_obj=m_trust_mismatch,
